    ...
    def render(self, eye_rays, num_bounces=3, sampling_type=IMPLICIT_BRDF_SAMPLING):
        ...
        if sampling_type == IMPLICIT_BRDF_SAMPLING:
            ...
            for bounce in range(num_bounces):
                ...
                shadow_hit_distances, shadow_normals, shadow_hit_ids = self.intersect(Shadow_rays)
                
                #Set L_e as 0 (inconsidered) for rays that went out of bounds
                L_e[shadow_hit_ids == -1] = np.array([0,0,0])
                                
                # Find which vectors are not zero (light hits)
                light_hit = np.any(L_e != 0, axis=1)
                
                
                #Update L_e (holds Le or zero based on if each ray hit an emitter)
                L_e = np.concatenate((np.array([obj.Le for obj in self.geometries]),
                                      np.array([0, 0, 0])[np.newaxis, :]))[shadow_hit_ids]
                ...
                #For still valid rays, if they hit nothing, set throughput to 0
                throughput[hit_mask & shadow_hit_ids == -1] = np.array([0,0,0])
                ...
                hit_mask[shadow_hit_ids == -1] = False
                

                #Update hits/normals for next bounce
                hit_points = Shadow_rays(shadow_hit_distances)
                normals = shadow_normals
                
                #Set eye_rays variable as Shadow_rays newly generated (for new bounce)
                eye_rays = Shadow_rays
