    ...
    def render(self, eye_rays, sampling_type=UNIFORM_SAMPLING):
        ...
        for l in range(len(self.lights)):
            if sampling_type == UNIFORM_SAMPLING:
                ...
                current_emmiter = self.lights[l]
                #Radius and center of current emmitter
                radius, center = current_emmiter.r, current_emmiter.c
                
                #Mask only consider points where eye rays hit
                hit_mask = ids != -1
                #Actual hit from eye rays to scene
                real_hits = hit_points[hit_mask]
                #Actual normals we case about
                real_normals = normals[hit_mask]
                
                #Origins of shadow rays, offsetted by the shadow_ray_o_offset in the normal direction
                Os = real_hits + shadow_ray_o_offset * real_normals          

                #Direction vector from hit_points to center of light source
                #Make the vector point from the offsetted origins to make sure shadow ray hit
                #either emitter or objects
                wc = center - Os
         
                #Building max angle that random shadow rays will be generated from
                sin_theta_max = radius / np.linalg.norm(wc, axis = 1)
                sin_theta_max = np.clip(sin_theta_max, 0, 1)
                theta_max = np.arcsin(sin_theta_max)[:, np.newaxis]
                
                #Normalize wc when building new orthonormal basis, but only do so after using its norm to
                #compute theta_max
                
                #Normalize wc
                wc /=np.linalg.norm(wc, axis=1, keepdims=True)
                          
                omega_e = 2 * np.pi * (1 - np.cos(theta_max))
                
                # Generate random samples for each grid element and for each sample
                xi1 = np.random.rand(normals[hit_mask].shape[0], 1)
                xi2 = np.random.rand(normals[hit_mask].shape[0], 1)
                
                # Calculate ray directions based on uniform sampling.                
                omega_z = 1 - (xi1 * (1 - np.cos(theta_max)))
                ...
                #Build new orthonormal basis where z-axis is wc
                
                #x-axis of new coordinate system cross wc with y
                u = np.cross(wc, np.array([0,1,0]))
                ...
                close_to_parallel = np.squeeze(u_norm < 1e-5)
                u[close_to_parallel] = np.cross(wc[close_to_parallel], np.array([1, 0, 0]))
                #Normalize u vectors
                ...
                T = np.stack((u,v,wc), axis=-2)
                
                transformed_omegas = np.einsum('ij,ijk->ik', omegas, T)

                #Shadow Ray instantiation                       
                Shadow_rays = Rays(Os = Os, Ds = transformed_omegas)
                #Get shadow ray hit distances
                shadow_hit_distances, _, shadow_hit_ids = scene.intersect(Shadow_rays)
                
                #Dot_product between normals and transformed_omegas
                Dot_product = np.sum(transformed_omegas * real_normals, axis=1, keepdims=True)

                #Set to 0 negative dot products
                Dot_product = np.maximum(0,Dot_product)
                                  
                #If the shadow ray hits the current emmiter, nothing is between the hit point
                #And the emmiter --> Visible hit point
                Visible = (objects[shadow_hit_ids] == current_emmiter).astype(int)
                #Visible = np.logical_or(objects[shadow_hit_ids] == current_emmiter, objects[shadow_hit_ids] == -1).astype(int)

                brdf_values = brdf_params[hit_mask]
                ...
                wo = -eye_rays.Ds[hit_mask][~diffuse_mask] 
                
                # Compute ωr for each hit point where alpha is not zero
                omega_r = 2 * np.sum(real_normals[~diffuse_mask] * wo, axis=1, keepdims=True) * real_normals[~diffuse_mask] - wo
                
                # Initialize the array to store fr values
                fr_values = np.zeros((len(alpha), 3))  # assuming the values are 3D (RGB or XYZ)
                
                #If alpha is 1, fr is pd/pi
                fr_values[diffuse_mask] = pd[diffuse_mask] / np.pi
                
                #If not diffuse, take max of 0 and dot between omega_r and normals
                cosine_term = np.maximum(0, np.sum(omega_r * transformed_omegas[~diffuse_mask], axis=1) ** alpha[~diffuse_mask])
                #Build fr_values for non diffuse surfaces    
                fr_values[~diffuse_mask] = (pd[~diffuse_mask] * (alpha[~diffuse_mask] + 1).reshape(-1,1) / (2 * np.pi)) * cosine_term.reshape(-1, 1)
                
                #Multiply by omega_e accounts for pdf of light, Visible to see if light ray actually hits points
                #And dot product to scale for observer
                visible_contribution = omega_e.squeeze() * Dot_product.squeeze() * Visible 
                #Multiply by L_e to scale by how "glossy" a surface is
                visible_contribution = visible_contribution[:, np.newaxis] * fr_values 
                        
                L[hit_mask] += visible_contribution * current_emmiter.Le
                ...
                # Generate random samples for each grid element and for each sample
                xi1 = np.random.rand(real_normals.shape[0], 1)
                xi2 = np.random.rand(real_normals.shape[0], 1)
                ...
                u_n = np.cross(real_normals[diffuse_mask], np.array([0,1,0]))
                ...
                u_n[close_to_parallel_n] = np.cross(real_normals[diffuse_mask][close_to_parallel_n], np.array([1, 0, 0]))
                #Normalize u vectors
                u_n = np.apply_along_axis(normalize, axis=1, arr=u_n)
                
                #Compute other vector in orthonormal basis using wc and u 
                v_n = np.cross(real_normals[diffuse_mask], u_n)
                v_n = np.apply_along_axis(normalize, axis=1, arr=v_n)
                
                #Build transformation matrix to coordinate around the cone for each light sphere
                T_diffuse = np.stack((u_n,v_n,real_normals[diffuse_mask]), axis=-2)
                ...
                Shadow_rays = Rays(Os = Os, Ds = transformed_omegas)
                #Get shadow ray hit distances
                shadow_hit_distances, _, shadow_hit_ids = self.intersect(Shadow_rays)
                
                #Dot_product between normals and transformed_omegas
                Dot_product = np.sum(transformed_omegas[~diffuse_mask] * real_normals[~diffuse_mask], axis=1, keepdims=True)
                ...
                #For all contribution, specular/diffuse has pd (or ps), L_e and Visible contribution
                L[hit_mask] += pd * current_emmiter.Le * Visible[:, np.newaxis] 
                #For phong contribution, add dot_product contribution appropriately
                L[hit_mask][~diffuse_mask] *= Dot_product.squeeze().reshape(-1, 1)
