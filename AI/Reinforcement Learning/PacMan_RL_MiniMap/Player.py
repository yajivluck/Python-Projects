...
from GameEntity import MapDescriptor, Fruit, Ghost
...
class MsPacManGame(object):
    ...
    def __init__(self, seed, display):
        ...
        self._ale.loadROM("MSPACMAN.BIN")
        self._reward = 0
    ...
    def available_actions(self):
        ...
        for action, move in [
            ...
            if 0 <= new_pos[0] < GameMap.HEIGHT:
                if self._map.map[new_pos] != MapDescriptor.WALL:
    ...
    def act(self, action):
        ...
        expected_reward = MapDescriptor.to_reward(self._map.map[next_pos])
        ...
        for _ in range(MAX_ACTION_COUNT):
            ...
            if self.game_over() or self._lives < old_lives:
                return MapDescriptor.to_reward(MapDescriptor.BAD_GHOST)
    ...
    def _update_state(self):
        ...
        self._fruit = Fruit.update(self._to_map_position(fruit[0]), fruit[1],
                                     fruit[0][0] != 0)
        ...
        self._ghosts = [
            Ghost.update(self._to_map_position(pos), ram)
    ...
    def _update_map(self):
        ...
        self._map.map[self._ms_pacman_position] = MapDescriptor.MS_PACMAN
        if self._fruit.exists:
            self._map.map[self._fruit.position] = MapDescriptor.FRUIT
        for ghost in self._ghosts:
            if ghost.state:
                self._map.map[ghost.position] = MapDescriptor.GOOD_GHOST
            else:
                self._map.map[ghost.position] = MapDescriptor.BAD_GHOST
